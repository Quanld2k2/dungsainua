...
public class Level18move : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
        ...
        foreach (UICollisionDetector other in otherUIDetectors)
        {
            if (other.gameObject != this.gameObject)
            {
                if (IsOverlapping(other.GetRectTransform()))
                {
                    ...
                    if ((other.name == "air1") && name == "dieukhien")
                    {
                        AudioManager.ins.play1shot(AudioManager.ins.level18[0]);

                        GameManager.ins.hint7 = true;

                        Level18.ins.air1.gameObject.SetActive(false);
                        Level18.ins.a_air.gameObject.SetActive(true);
                        Level18.ins.a_air.AnimationState.SetAnimation(1, "animation", true);

                        Level18.ins._8.gameObject.SetActive(true);
                        Level18.ins._8.gameObject.GetComponent<RectTransform>().DOAnchorPos(new Vector3(250f, -460f, 0f), 0.5f).SetDelay(0.4f).OnComplete(() =>
                        {
                            Level18.ins._8.gameObject.SetActive(false);
                    ...
                    else if ((other.name == "door1") && name == "key")
                    {
                        GameManager.ins.hint6 = true;

                        Level18.ins.door1.gameObject.SetActive(false);
                        Level18.ins._3.gameObject.SetActive(true);
                        Level18.ins.door2.gameObject.SetActive(true);

                        this.gameObject.GetComponent<RectTransform>().anchoredPosition = initialPosition;
                        this.gameObject.SetActive(false);
                        return collided;
                    }
                    else if ((other.name == "bear") && name == "knife")
                    {
                        Level18.ins.bear.gameObject.SetActive(false);

                        Level18.ins.a_bear.gameObject.SetActive(true);
                        Level18.ins.a_bear.AnimationState.SetAnimation(1, "animation", false);
